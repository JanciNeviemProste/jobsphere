...
/**
 * Shared body for every `error.tsx` route boundary.
 *
 * The App Router requires a real file per segment, but those files used to be
 * 11 byte-identical copies differing only in the Sentry `location` tag — so a
 * single fix (e.g. the locale-aware "go home" link that only `employer` had)
 * silently applied to one of them. Each boundary now delegates here and passes
 * its own `location`.
 */
export function RouteError({
  ...
  reset: () => void
  /** Sentry tag identifying which route segment blew up. */
  location: string
}) {
